...
출처 : [https://vector.dev](https://vector.dev){:target="_blank"} 메인 페이지
...
vector에서 데이터는 event라고 표현한다. decoding하면 event로 변환되고 이 event를 가공할 수 있다. 참고: [Data model](https://vector.dev/docs/about/under-the-hood/architecture/data-model/){:target="_blank"} 

[Vector Remap Language (VRL)](https://vector.dev/docs/reference/vrl/){:target="_blank"} 언어로 데이터를 가공할 수 있다.

예시로, del(.필드명)로 필요없는 필드를 삭제할 수 있다.

사용 가능한 함수는 아래 References에서 확인할 수 있다.

`sink`

여기서 acknowledgements를 활성화하면 kafka offset으로 데이터 처리를 관리한다. ( at-least-once 보장 )

acknowledgements는 sink에서 활성화하고 source는 ack를 유지하기 위한 설정을 한다. kafka 경우는 consumer group을 설정하면 된다.

[Vector - Guarantees](https://vector.dev/docs/about/under-the-hood/guarantees/#faq-at-least-once){:target="_blank"}

기본적으로 acknowledgements를 설정하지 않는 경우, source에서 데이터를 받아간 순간 데이터 처리가 되었다고 판단한다.
...
Athena에서 쿼리 가능한 형태로 변경하려면 transform으로 event(.)를 encode_json으로 문자열로 처리하고 codec: "text"로 설정해야 한다.
...
```sql
[
{"device_name":"device_a","headers":{},"message_key":"test","offset":62643,"one":-10,"partition":1,"source_type":"kafka","three":42,
"timestamp":"2024-12-28T17:39:19.098Z","topic":"vector-test","two":-48},
...
]
```
...
참고: aggregator에 configmap을 활용해서 처리해보려 했으나 파이프라인 생성 시 일회성 POD이 실행되는데 이 때 desc_file 유무를 체크해서 오류가 났다.
...
하나의 파이프라인에서 여러 개의 source와 sink를 적용할 수 있다. 주기가 느린 데이터의 경우에는 하나의 파이프라인에 추가로 적용해도 무리가 없을 것으로 보인다.
